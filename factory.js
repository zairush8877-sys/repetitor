...
const CONTENT_DIR = path.resolve(process.env.MEDIA_CONTENT_DIR || path.join(__dirname, 'content'));
const USED_FILE = path.join(CONTENT_DIR, 'used-topics.json');

const POSTS_PER_WEEK = 7; // Автор 14.09.2026: публикации ежедневно.
const POST_DAYS = [0, 1, 2, 3, 4, 5, 6];
...
const startDate = startArg || new Date().toLocaleDateString('en-CA', { timeZone: 'Europe/Moscow' });
// Calendar dates are UTC day values, independent of the Windows/runner time zone.
...
function postDates(from, totalPosts) {
  ...
  while (dates.length < totalPosts) {
    if (POST_DAYS.includes(d.getUTCDay())) dates.push(new Date(d));
...
// а не туда, куда их поставит взвешенная очередь. Такой пост вытесняет обычный.
{
  ...
  for (const p of pinnedList) {
    ...
    const entry = {
      date: p.date,
      day: DAYS_RU[new Date(p.date + 'T00:00:00Z').getUTCDay()],
      rubric: p.rubric,
      format: p.format,
      topic: p.topic,
      pinned: true,
...
for (const p of plan) {
  const mark = p.pinned ? ' 📌' : '';
  md += `| ${fmt(new Date(p.date + 'T00:00:00Z'))} | ${p.day} | ${p.rubric}${mark} | ${p.format} | ${p.topic} |\n`;
}
const anchors = plan.filter(p => p.pinned);
if (anchors.length) {
  md += `\n📌 — привязано к дате:\n\n`;
  for (const p of anchors) md += `- ${fmt(new Date(p.date + 'T00:00:00Z'))} ${p.topic} — ${p.why}\n`;
}
...
for (const p of auto) md += `- ${fmt(new Date(p.date + 'T00:00:00Z'))} — ${p.topic}\n`;
if (!auto.length) md += `- (нет)\n`;

md += `\nНужна съёмка — снять за один заход:\n\n`;
for (const p of shoot) md += `- ${fmt(new Date(p.date + 'T00:00:00Z'))} — ${p.topic}\n`;
...
console.log(`\nПервые публикации:`);
for (const p of plan.slice(0, 7)) console.log(`  ${fmt(new Date(p.date + 'T00:00:00Z'))} ${p.day} — [${p.rubric}] ${p.topic}`);
